...
func (pd *ProductDB) GetProduct(id string) (*entity.Product, error) {
	...
	err := pd.db.QueryRow("SELECT id, name, price, category_id, imageURL FROM products WHERE id = ?", id).
...
func (pd *ProductDB) CreateProduct(product *entity.Product) (*entity.Product, error) {
	_, err := pd.db.Exec("INSERT INTO products (id, name, price, category_id, imageURL) VALUES (?, ?, ?, ?, ?)",
